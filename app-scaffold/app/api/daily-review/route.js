...
export async function GET(request) {
  ...
  if (sourceData.basDt !== todayKstBasDt()) {
    // [2026-09-10 추가] 재성님 리포트 — 스케줄(cron-job.org)은 분명 저녁
    // 9시(한국시간)로 정확히 설정돼 있는데도 이 조건이 걸려서 건너뛴 사례가
    // 있었음. 코드상으로는 두 값(오늘 실제 날짜 vs 데이터의 날짜)이 같아야
    // 정상인데 실제로는 달랐던 거라, 그 이유를 다음번엔 바로 알 수 있도록
    // "오늘로 계산된 값"과 "서버가 생각하는 현재 시각"까지 reason에 같이
    // 남겨둠 — 전에는 basDt 하나만 보여서 뭐가 문제인지 추측만 할 수 있었음.
    return Response.json({
      ok: false,
      skipped: true,
      reason: `basDt(${sourceData.basDt})가 오늘(KST, ${todayKstBasDt()}) 날짜와 다릅니다 — 휴장일로 추정, 건너뜁니다. (서버 시각: ${new Date().toISOString()})`,
    });
  }
  const hasData =
