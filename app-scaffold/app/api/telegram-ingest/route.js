...
// [2026-09-24 추가] 재성님 리포트 — 카드 제목이 "[종목이보영] 이수페타시스,
// 고부가 AI 기판으로 성장 가속"처럼, 언론사 연재 코너 이름이 제목 앞에 그대로
// 붙어서 나오는 문제.
//
// 원인은 두 가지였습니다.
//  ① 위 NOISE_LEADING_TAGS는 "지울 태그"를 하나씩 등록하는 방식이라,
//     [종목이보영]처럼 처음 보는 코너 이름은 그냥 통과합니다. 그런데 연재
//     코너 이름은 언론사마다 수십 가지라 매번 한 줄씩 추가하는 걸로는 끝이
//     나지 않습니다.
//  ② 텔레그램 메시지에 제목이 없어서 원문 기사에서 제목(og:title)을 대신
//     가져오는 경우에는, 그 제목에 태그 청소를 아예 안 하고 있었습니다.
//     [종목이보영] 같은 코너 이름이 바로 여기로 들어옵니다.
//
// 그래서 방식을 뒤집었습니다. 이제는 "남길 태그"만 정해두고(KEEP_LEADING_TAGS),
// 그 목록에 없는 대괄호 태그는 제목 앞에서 전부 지웁니다. 재성님이 예전에
// "특징주, 속보 이런 글씨는 그대로 가져와도 돼"라고 하신 것들은 그대로 남고,
// 언론사 코너 이름·기자 브랜드 같은 건 새로 나타나도 자동으로 지워집니다.
// 나중에 "이 태그도 남겨줘" 하실 게 생기면 아래 배열에 한 단어만 추가하면 됩니다.
const KEEP_LEADING_TAGS = [
  "속보",
  "단독",
  "특징주",
  "급등주",
...
// 제목 맨 앞의 대괄호 태그 하나를 잡아내는 패턴. 대괄호는 []뿐 아니라
// 【】 형태로 쓰는 언론사도 있어서 둘 다 인식합니다.
const LEADING_TAG_RE = /^\s*[[【]\s*([^[\]【】]{1,20}?)\s*[\]】]\s*/;

function stripBrandLeadingTags(title) {
  const original = (title || "").trim();
  let rest = original;
  const kept = [];
  // "[종목이보영][단독] 제목"처럼 태그가 여러 개 붙는 경우가 있어서 최대
  // 4개까지 훑어보고, 남길 태그만 순서대로 다시 앞에 붙여줍니다.
  for (let i = 0; i < 4; i++) {
    const match = rest.match(LEADING_TAG_RE);
    if (!match) break;
    const tag = match[1].replace(/\s+/g, "");
    if (KEEP_LEADING_TAGS.includes(tag)) kept.push(`[${tag}]`);
    rest = rest.slice(match[0].length).trim();
  }
  // 태그를 다 떼고 나니 제목이 통째로 사라지는 경우(태그만 있는 제목)에는
  // 원래 제목을 그대로 씁니다 — 빈 제목으로 게재되는 게 더 나쁩니다.
  if (!rest) return original;
  return (kept.length ? `${kept.join("")} ${rest}` : rest).trim();
}
...
export async function POST(request) {
  ...
  // [2026-09-24 추가] 제목 앞에 붙은 언론사 연재 코너 이름·기자 브랜드 태그를
  // 여기서 한 번에 지웁니다(stripBrandLeadingTags 주석 참고). 텔레그램에서 온
  // 제목이든 원문 기사에서 가져온 제목이든 이 줄을 반드시 거치게 해서,
  // "[종목이보영] ..." 같은 게 카드에 그대로 노출되지 않게 합니다. 이 자리에
  // 둔 이유는, 뒤따르는 종목 매칭·AI 필터도 깨끗해진 제목을 쓰게 하기 위함입니다.
  title = stripBrandLeadingTags(title);
