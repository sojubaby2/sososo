...
// Vercel terminates a function that runs past this many seconds. Processing
// articles one-by-one in sequence was blowing past the platform default and
// causing the scheduler's calls to fail with a timeout — combined with
// running articles concurrently below, this gives plenty of headroom.
export const maxDuration = 60;

async function processArticle(keyword, article, redis) {
...
export async function GET(request) {
  ...
  for (const keyword of WATCHED_KEYWORDS) {
    ...
    // Process every article for this keyword concurrently rather than
    // waiting on each one in turn — this is what actually fixes the
    // timeout (20+ sequential Claude calls easily blew past the time
    // budget; run in parallel, the wall-clock time is roughly just the
    // slowest single article instead of the sum of all of them).
    const results = await Promise.all(articles.map((article) => processArticle(keyword, article, redis)));
    log.push(...results);
  }
